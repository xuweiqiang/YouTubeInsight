...
2. **自动分析**：在设置中导入 Google OAuth 桌面应用 JSON 并绑定账号，
   客户端会检查最近24小时的订阅更新，并按订阅数量自动安排后续刷新。
...
- 应用运行期间最短每15分钟刷新一次，并根据订阅频道数量自动延长间隔以控制
  YouTube API 配额；也可随时手动刷新。
...
5. 后续保持应用运行即可自动刷新，也可点击“立即刷新”。刷新最短间隔为
   15分钟，订阅较多时会自动延长以避免耗尽 YouTube API 配额。
...
`1.6.7（构建 20260728103000）`。相同内容重复构建时保持不变，也可以通过
...
从 1.6.5 起，视频信息读取遇到临时网络或提取器错误时会自动重试，并在失败时
显示 `yt-dlp` 返回的具体原因，方便区分公开视频限制、登录验证和网络问题。
从 1.6.6 起，尚未开播的直播会被暂缓而不是计为失败，开播后由定时扫描重新分析。
从 1.6.7 起，自动扫描会根据订阅频道数量估算请求成本，在每日默认配额内动态
调整间隔。检测到 `quotaExceeded` 后，本轮扫描会立即停止，不再重复发送无效
请求；YouTube 在太平洋时间午夜重置配额后，客户端会自动恢复。手动粘贴链接
使用 `yt-dlp`，不消耗 YouTube Data API 配额。
...
这只影响订阅频道的自动发现，不影响手动粘贴链接分析。客户端会停止本轮 API
请求，并在太平洋时间午夜重置配额后自动恢复。1.6.7及更高版本会按订阅频道
数量动态调整扫描间隔，降低再次耗尽每日配额的概率。
